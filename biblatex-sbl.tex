...
By default SBL recommends that full pages for a chapter or article be included
in the bibliography and only the page or pages under consideration be
referenced in the first citation.
...
This entry option controls how a title in a non-Latin alphabet is formatted,
if you wish to cite by this title rather than a translated title. The
translated title is then placed in |translatedtitle|, |translatedbooktitle| or
|translatedmaintitle|.
...
\begin{function}{shorthandformat}
  \begin{syntax}
    = roman \textbar italic\hfill Default: italic
  \end{syntax}
\end{function}

The format of a |shorthand| field in SBL style needs to match what the
abbreviation points to. So if the |shorthand| refers to the title of the work,
it should be printed in italics, whereas if the |shorthand| refers to the
authors of the work it it should be set in a Roman font. By default the
...
|maintitle| because the |title| is overly generic (e.g., \emph{Books 1–3}). In
these cases, set use |options = {usetitle=false}| to place the |title| after
the |maintitle|.
...
Generally, ancient works which are part of a text collection include the text
collection in the citation. One exception to this is SBL's treatment of
ancient epistles and homilies,\footnote{See §6.4.4 of the
handbook.} where the
collection does not appear in the citation, but it still needs to appear in
the bibliography. This behaviour can be achieved by placing the text
