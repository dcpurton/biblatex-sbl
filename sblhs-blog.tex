...
\NewDocumentCommand{\exampleparencite}{sd()om}{%
  \par
  \IfNoValueTF{#3}
    {\texttt{\textbackslash parencite\IfBooleanT{#1}{*}\{#4\}}\par
     \@afterheading
     \IfNoValueF{#2}{\quad #2.\space}%
     \cite{#4}}
    {\texttt{\textbackslash parencite\IfBooleanT{#1}{*}[#3]\{#4\}}\par
     \@afterheading
     \IfNoValueF{#2}{\quad #2.\space}%
     \IfBooleanTF{#1}
       {\parencite*[#3]{#4}}
...
\exampleparencite[(50)]{philo:cher}
\exampleparencite[(1.6)]{philo:QG}
